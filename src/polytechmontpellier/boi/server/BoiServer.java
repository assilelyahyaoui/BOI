...
public class BoiServer implements Observer{
	 ...
	 */
	private void handleMessageFromClient(Object msg, ConnectionToClient client) {
		...
		if(message.startsWith("#login")) { 
			...
			if(this.facade.login(message.split("||")[1], message.split("||")[2])) {
				...
				this.observableServer.sendToAllClients("LOGGED_IN");
			}else {
				this.observableServer.sendToAllClients("BAD_CREDENTIALS");
			}
			...
			this.observableServer.sendToAllClients("UNAUTHORIZED");
		}
	}
		...
		public void update(Observable o, Object arg) {
			...
			if(((String)msg.getMessage()).startsWith(ObservableOriginatorServer.CLIENT_EXCEPTION)){
				...
				switch((String) msg.getMessage()){
					...
					default : this.handleMessageFromClient(msg.getMessage(),msg.getOriginator());
